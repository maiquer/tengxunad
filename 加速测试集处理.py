...
test_list = pd.read_csv('final_select_test_request.out',header=None,sep='\t')
test_list.columns = ['ad_id','request_list']

test_log = pd.read_csv('test_tracklog_20190423.last.out',header=None,sep='\t')
...
res.to_csv('./train/max_total.csv',index=None,header=None)  
...
t = test_simple['request_list'].str.split('|')
t = pd.DataFrame(t)
target = []
...
test_simple['target_epcm'] =  target

test_change = test_simple[['sample_id','ad_id','target_epcm']]

day422 = pd.read_csv('./train/0422.csv',header=None)
...
submission.to_csv('./res/submission.csv',index=None,header=None)   